...
;; Use the Left Windows key as Meta
(setq w32-lwindow-modifier 'meta)

;; Use the Right Windows key as Meta
...
;; Common directories
(setq gnus-home-directory "/home/italo/"
      gnus-cache-directory (concat gnus-home-directory ".cache/emacs")
      gnus-cache-active-file (concat gnus-home-directory ".cache/emacs/active")
      config-dir (concat gnus-home-directory ".config/")
      downloads-dir (concat gnus-home-directory "Downloads/")
      org-directory (concat gnus-home-directory "org")
      bread-dir (concat gnus-home-directory ".emacs.d/")
      repos-dir (concat gnus-home-directory "repos/")
...
(defun hf-token (which)
  "Load HF_TOKEN from 1Password into Emacs' environment.
Interactively prompts for WHICH: personal, oc-read, or oc-write."
  (interactive
   (list (completing-read "HF token: " '("personal" "oc-read" "oc-write") nil t)))
  (let* ((ref (pcase which
                ("personal" "op://3jjpmvudv4ecxo6shvbtv47rqa/2igtoregnxwp5vg2kqnqbbw6si/credential")
                ("oc-read"  "op://3jjpmvudv4ecxo6shvbtv47rqa/robqktsvh2yxualpcvb4f3bn5i/credential")
                ("oc-write" "op://3jjpmvudv4ecxo6shvbtv47rqa/6x26qhkb2oecxevfyirnpo254e/credential")
                (_ (user-error "Usage: hf-token {personal|oc-read|oc-write}"))))
         (token (string-trim (shell-command-to-string (format "op read %s" (shell-quote-argument ref))))))
    (setenv "HF_TOKEN" token)))
...
(use-package agent-shell
  ...
  (add-to-list 'exec-path "/home/italo/.npm-global/bin")
